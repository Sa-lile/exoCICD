...
app.get('/', (req, res) => {
  res.render('calculate', { error: null, result: null, du: null, donne: null, rendu: null, etatCaisse });
});

// Return caisse state as JSON 
app.get('/etat-caisse', (req, res) => {
  return res.json(etatCaisse);
});
...
app.post('/calculate', (req, res) => {
  try {
    ...
    if (rendu && rendu.error) {
      return res.render('calculate', { error: rendu.error, du: montantDu, donne: montantDonne, result: montantARendre, rendu: null });
    }
...
if (etatCaisse && typeof etatCaisse === 'object') {
  ...
  // console.log('Etat de la caisse triée :', valeurs);
} else {
